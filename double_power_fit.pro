pro double_power_fit, radsamp, Nesamp, A, chisq, weighted=weighted
...
; Set weights:
  if NOT keyword_set(weighted) then weights = 0.*Nesamp + 1.
  if     keyword_set(weighted) then weights =    Nesamp / mean(Nesamp)

; Fit:
  yfit_func = CURVEFIT(radsamp, Nesamp, weights, A, SIGMA,$
