...
; Read the list with the field-lines  
  ...
  N_pt_max = 15000              ; hay que encontrar un valor adecuado...

; Aca se crean varios arrays 
  x_A   = dblarr(N_fl,N_pt_max) - 666.
  y_A   = dblarr(N_fl,N_pt_max) - 666.
  z_A   = dblarr(N_fl,N_pt_max) - 666.
  rad_A = dblarr(N_fl,N_pt_max) - 666.
  lat_A = dblarr(N_fl,N_pt_max) - 666.
  lon_A = dblarr(N_fl,N_pt_max) - 666.
  Npt_v = intarr(N_fl)      
  if keyword_set(aia) then begin
     Ne_aia_A    = fltarr(N_fl,N_pt_max) - 666.
     Tm_aia_A    = fltarr(N_fl,N_pt_max) - 666.
     index_aia_A = intarr(N_fl,N_pt_max) - 666
  endif
  if keyword_set(euvia) then begin
     Ne_euvia_A    = fltarr(N_fl,N_pt_max) - 666.
     Tm_euvia_A    = fltarr(N_fl,N_pt_max) - 666.
     index_euvia_A = intarr(N_fl,N_pt_max) - 666
  endif
  if keyword_set(euvib) then begin
     Ne_euvib_A    = fltarr(N_fl,N_pt_max) - 666.
     Tm_euvib_A    = fltarr(N_fl,N_pt_max) - 666.
     index_euvib_A = intarr(N_fl,N_pt_max) - 666
  endif
  if keyword_set(eit) then begin
     Ne_eit_A    = fltarr(N_fl,N_pt_max) - 666.
     Tm_eit_A    = fltarr(N_fl,N_pt_max) - 666.
     index_eit_A = intarr(N_fl,N_pt_max) - 666
  endif
; Asi hay que seguir con los demas instrumentos
